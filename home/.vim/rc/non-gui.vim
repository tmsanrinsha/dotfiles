...
" vim からの制御シーケンスの使用例 クリップボードからの貼り付け時に自動インデントを無効にする
" https://ttssh2.osdn.jp/manual/ja/usage/tips/vim.html
if exists('&t_BE')
  if &term =~? 'screen'
    " :help term-bracketed-paste
    let &t_BE = "\e[?2004h"
    let &t_BD = "\e[?2004l"
    let &t_PS = "\e[200~"
    let &t_PE = "\e[201~"
  ...
  function! s:toggle_bracketed_paste_mode() abort
    if &t_BE == ''
      let &t_BE = "\e[?2004h"
    else
      let &t_BE = ''
  ...
  noremap <expr> <S-F1> <SID>toggle_bracketed_paste_mode()
  inoremap <expr> <S-F1> <SID>toggle_bracketed_paste_mode()
...
elseif &term =~? 'screen' || &term =~? 'xterm'
  ...
  inoremap <expr> <F13> <SID>xterm_paste_begin()
endif
